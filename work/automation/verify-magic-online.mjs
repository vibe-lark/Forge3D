...
const chineseName = await appFrame.locator('[data-asset-id="detailed-satellite-dish"] strong').textContent();
await appFrame.locator('[data-asset-id="detailed-satellite-dish"]').click();
await appFrame.waitForFunction(() => {
  ...
  return name === '详细卫星天线' && meshes && meshes !== '—' && source.includes('妙笔 TOS');
}, null, { timeout: 90000 });
...
const failures = [];
if (count !== 77) failures.push(`asset count ${count}`);
if (chineseName?.trim() !== '详细卫星天线') failures.push(`Chinese name ${chineseName}`);

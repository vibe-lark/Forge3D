...
const chineseName = await appFrame.locator('[data-asset-id="double-bed"] strong').textContent();
await appFrame.locator('[data-asset-id="double-bed"]').click();
await appFrame.waitForFunction(() => {
  ...
  return name === '双人床' && meshes && meshes !== '—' && source.includes('妙笔 TOS');
}, null, { timeout: 90000 });
...
const failures = [];
if (count !== 95) failures.push(`asset count ${count}`);
if (chineseName?.trim() !== '双人床') failures.push(`Chinese name ${chineseName}`);
